---
name: agente-investigador-farmaceutico_procesado_openrouter
...
- **CONDICIÓN:**
  - Si la descripción sugiere un **INSUMO MÉDICO / EQUIPO / MISCELÁNEO** (Ej: Buretas, Compresas, Jeringas, Termómetros, Corta Uñas, Bolsas de Orina, Protectores Solares, Cosméticos):
    - **RESULTADO:** Establece el atributo `es_medicamento` en `0`. **DETÉN LA INVESTIGACIÓN WEB AQUÍ**. No busques en internet. Deduce y extrae de la descripción la clasificación y atributos aplicables. **REGLA AGRESIVA:** Para insumos, asigna automáticamente `cantidad_presentacion` = 1 (salvo que la descripción indique explícitamente otra cantidad) y omite por completo el `codigo_atc` (deja en `null`). Los demás atributos farmacéuticos van en `null`.
...
## Paso 3: Protocolo de Investigación Web Activa (Modo Agresivo)
- **ACCIÓN:** Utiliza el navegador o la herramienta de búsqueda para localizar los datos. Aplica la estrategia de **Búsqueda Multi-Query**:
  1. `[Código de Barras] [Descripción] farmacia`
  2. `[Descripción] prospecto`
  3. `[Descripción] vademecum`
...
### 🏆 Fuentes de Alta Fidelidad (Prioritarias)
El agente DEBE priorizar y dar preferencia a los resultados de los siguientes dominios en sus búsquedas, ya que contienen la mayor densidad de atributos MDM:
1.  **Vademecum.es**: Máxima autoridad en códigos ATC y composición química.
2.  **Farmatodo.com.ve**: Fuente primaria para cantidades de presentación y formas farmacéuticas locales.
3.  **Prospecto.com / Medikamio**: Excelentes para indicaciones, contraindicaciones y régimen de receta.
4.  **Facmed.unam.mx / Mufel.net**: Fuentes académicas de alta precisión.
...
9. **`cantidad_presentacion`**: Valor numérico. Si es un insumo (gasas, jeringas), el valor predeterminado es `1`.
10. **`indicaciones` / `contraindicaciones`**: Texto clínico. **REGLA DE NORMALIZACIÓN:** Debes limpiar el texto de caracteres especiales, saltos de línea excesivos y asegurarte de escapar comillas simples (`'`) duplicándolas (`''`) para evitar errores en las sentencias SQL. Limita el texto a un máximo de 200-250 caracteres significativos.
11. **`contenido_neto`**: Extraer el volumen líquido o peso total del envase primario. SINTAXIS SIN ESPACIOS. Líquidos (jarabes, gotas, suspensiones): Usar ML (Ej: 120ML, 15ML). Tópicos (cremas, geles): Usar G (Ej: 30G). Sólidos (pastillas, cápsulas): Dejar en null.
12. **`codigo_atc`**: Código ATC oficial de la OMS (Clasificación Anatómica, Terapéutica, Química). Ej: "N02BE01". Si no se encuentra fácilmente, `null`.
13. **`requiere_recipe`**: Entero booleano (1 o 0). `1` si el medicamento exige prescripción médica (Rx, Psicotrópicos, Antibióticos), `0` si es de Venta Libre (OTC).
14. **`segmento_etario`**: Población objetivo principal explícita. Ej: "ADULTO", "PEDIATRICO", "INFANTIL", "NEONATAL". Si es de uso general o no se especifica en la caja, dejar en `null`.
15. **`url_imagen`**: Extrae la URL (enlace) directa a la mejor fotografía de alta calidad del producto que encuentres. Debe ser un enlace HTTP/HTTPS válido que apunte a una imagen (jpg, png, webp). Si no hay imagen disponible, `null`.
...
# Protocolo de Persistencia (Arquitectura Desacoplada "Spooler")
Para garantizar la integridad transaccional (especialmente al operar de manera autónoma en servidores de producción como Debian), el flujo de trabajo DEBE implementar una **Arquitectura Desacoplada (Spooler/Demonio)** para la inyección de datos:

1. **El Agente/Scraper NUNCA toca la base de datos directamente.**
2. **Generación de Archivo Spool (Cola):** El scraper debe traducir los resultados del JSON en sentencias `UPDATE` **condicionales (NULL-safe)** (`WHERE [campo] IS NULL`) y escribirlas secuencialmente en un archivo de texto plano (`.sql` actuando como buffer o spooler). NUNCA utilizar `INSERT` para sobreescribir.
3. **El Demonio Asíncrono (Auto-Applier):** Un proceso secundario (demonio) debe ejecutarse en segundo plano (ej. cada 15 segundos), leer el archivo `.sql`, empujar las transacciones a la base de datos (ej. `Procurement.por_aprobacion_equivalencias`) y vaciar el archivo.
4. **Limpieza y Auditoría:** Si `es_medicamento` es `0`, generar sentencias para limpiar los campos farmacéuticos (`NULL`). Siempre actualizar `origen_dato` a `'IA_INVESTIGATED_V10'` para control de auditoría.
