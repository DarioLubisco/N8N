---
name: conciliador-mdm-farmaceutico_procesado_antigravity
...
**NOTA:** Esta skill opera bajo el enfoque "Opción 2" (Agentic). Eres tú (Antigravity) quien orquesta la ejecución local, controlando las búsquedas y aplicando los scripts SQL/Python directamente mediante tus herramientas, garantizando una consolidación MDM idéntica o superior sin depender de la ejecución automática paralela de LLMs externos.
...
## Fase B: Conciliación de Catálogos Automática (Mapeo a IDs por Hermes)
Para garantizar la independencia de un LLM en el cruce de catálogos, **Hermes (o el orquestador)** debe ejecutar este proceso de 3 pasos de forma autónoma una vez que los datos descriptivos (`_Des`) están en la base de datos:

1. **Ejecución del Conciliador Python (Fuzzy Mapper):**
   - **Comando:** `python MDM_Fuzzy_Mapper.py`
   - **Dependencias:** Requiere `rapidfuzz`, `pyodbc`, `python-dotenv`.
   - **Acción Interna:** El script extraerá los registros huérfanos, normalizará los textos (quitando acentos/caracteres especiales) y usará distancia de Levenshtein para compararlos contra `dbo.principio_activos`.

2. **Aplicación Automática de Cambios SQL (Match >= 90%):**
   - El script genera un archivo llamado `MDM_Phase4_Fuzzy_Mapping.sql`.
   - **Acción:** Hermes debe leer este archivo y ejecutar los `UPDATE` en la base de datos. Estos son los matches de alta confianza.

3. **Manejo de Faltantes Reales (Match < 90%):**
   - El script genera un archivo `Faltantes_Reales.json` con los descriptores que no lograron un match seguro.
   - **Acción:** Hermes debe leer este JSON y enviarlo por la cola de revisión (Telegram/Chatwoot o Dashboard) para que un humano (o un proceso secundario de IA semántica como `ai_mapper.py`) decida si se crea un nuevo ID en el catálogo maestro.

---

# Sincronización Estricta de Procedimientos (Heurística vs IA)

Es un mandato arquitectónico que **ambos procedimientos** (el mapeador local heurístico y el mapeador semántico de IA) compartan **exactamente la misma lógica y conocimiento de dominio**:

1. **Reparación de Mojibake / Corrupción de Caracteres:**
   Ambos scripts deben pre-procesar y corregir artefactos de codificación antes de buscar coincidencias:
   - "Cafena" -> "Cafeína"
   - "sdica" -> "sódica"
   - "solucin" -> "solución"
   - Terminaciones en "cido" / "xido" -> "ácido" / "óxido" (ej. "xido de zinc").
2. **Matriz de Sinónimos y Compuestos Multingrediente:**
   El diccionario de sinónimos debe estar inyectado tanto en el código Python de `MDM_Fuzzy_Mapper.py` como en el **Prompt del Sistema** de `ai_mapper.py` para asegurar consistencia.
   Ejemplos críticos:
   - `PARACETAMOL CLORFENAMINA DEXTROMETORFANO` = `ACETAMINOFEN CLORFENAMINA DEXTROMETORFANO`
   - `PARACETAMOL CAFEINA` = `ACETAMINOFEN CAFEINA`
   - `SULFACETAMIDA SODICA TRIETANOLAMINA UREA` -> fuerza vinculación a `SULFACETAMIDA SODICA`.

Cualquier actualización a las reglas de negocio (nuevos sinónimos o reglas de limpieza) **debe replicarse obligatoriamente** en ambos archivos `.py`.
