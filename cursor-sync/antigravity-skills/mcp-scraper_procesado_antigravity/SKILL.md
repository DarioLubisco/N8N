---
name: mcp-scraper_procesado_antigravity
...
**NOTA:** Esta skill opera bajo el enfoque "Opción 2" (Agentic). Esto significa que tú (Antigravity) ejecutas este proceso de manera autónoma y local, reemplazando la necesidad de llamar a una API LLM externa como OpenRouter. Aunque el proceso es secuencial (lote por lote a través del chat), la calidad de la extracción de datos y el cumplimiento de las reglas serán exactamente iguales o superiores.
...
## 1.5. Estrategia de Búsqueda Agresiva (Multi-Query)
- Para maximizar la recolección de atributos críticos (Cantidades, ATC, Indicaciones), aplica búsquedas agresivas si los datos no se encuentran a la primera:
  1. `[Código de Barras] [Descripción] farmacia`
  2. `[Descripción] prospecto`
  3. `[Descripción] vademecum`
- **Excepción de Insumos:** Si identificas que se trata de equipo o insumos médicos (no fármacos), asume `cantidad_presentacion = 1` y omite el Código ATC sin malgastar recursos de Scrapling.
...
# Protocolo de Automatización de Inserción (Persistencia)
Los hallazgos extraídos mediante Scrapling deben persistirse de manera automática y segura:
1. Generar sentencias SQL `UPDATE` de forma continua (ej. un script o daemon como `auto_apply_sql.py` que aplique lotes periódicamente).
2. Cada sentencia generada debe ser **NULL-safe**, lo que significa que solo puedes inyectar datos utilizando condiciones como `WHERE [campo_objetivo] IS NULL`. Nunca sobrescribas los valores previamente curados a mano o insertados.
